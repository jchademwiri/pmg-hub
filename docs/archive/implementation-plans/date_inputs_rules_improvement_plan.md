...
- _Improvement:_ Add a closed period check in `updateInvoice` to ensure historical invoices cannot be modified into a closed period (maintains consistency with `createInvoice`).
...
- _Improvement:_ Add a closed period check in `updateQuotation` to ensure consistency with `createQuotation`.
