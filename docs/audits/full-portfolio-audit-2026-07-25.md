...
`packages/db/src/queries/general.ts:53-65`, `getExpensesByDivision()`, has **no date/period filter at all** — it sums *all-time* expenses per division:
...
---
*Read-only audit. Live checks ran against production URLs provided by the repo owner, who signed in themselves for the authenticated `admin` session — no credentials were entered by the assistant. Authenticated `portal` audit remains open pending a fix to the Clients-page hang.*
