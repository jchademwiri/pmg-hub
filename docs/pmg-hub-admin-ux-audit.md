...
- Fixed chart layout shift with `min-h` containers in `reports-tabs.tsx`
- Renamed "New Tender" to "New Project" in project overview
- Added hover effects to project summary cards (`hover:shadow-md` + `-translate-y-0.5`)
- Renamed "Active Tenders" to "Active Projects" in summary labels
...
- Added `error.tsx` files for accounting sub-routes using `RouteError`
- Removed unused `useRouter` import from `quotes-client.tsx`
...
**Status:** 🔲 Backlog (requires Suspense boundary architecture changes)
...
**Status:** ✅ Fixed in Backlog Batch 2 (`9f23903`)
...
**Recommendation:** Created reusable `Pagination` component with page numbers, ellipsis, and prev/next buttons. Applied to both invoices and quotes list pages.
...
### 4.7 🟢 `billing/accounts/page.tsx` has a full implementation — no stub
...
No action needed — this was incorrectly identified as a stub.
...
**Status:** 🔲 Backlog (large refactoring opportunity)
...
**Status:** 🔲 Backlog (consistency pass needed)
...
**Status:** 🔲 Backlog (requires architecture changes)
...
**Status:** ✅ Fixed in Backlog Batch 3 (`2a29044`)

**Finding:** Added `loading.tsx` files for chart-of-accounts, general-ledger, and trial-balance sub-routes. Added `error.tsx` files for the same sub-routes using the shared `RouteError` component.
...
- Help text when no export is selected

No action needed — this was incorrectly identified as missing descriptions.
...
**Status:** ✅ Already implemented (uses `router.push` with URL params)

**Finding:** The leads filter bar already writes filter state to URL search params, matching the billing filter bar pattern.
...
**Status:** ✅ Already implemented (has `confirm()` dialog)

**Finding:** The convert-to-client button already includes a `confirm()` dialog explaining what will happen during conversion.
...
**Finding:** The primary action button was rendered as the first child of the content area. The label was also "New Tender" instead of "New Project".

**Recommendation:** Renamed button to "New Project". Button positioning remains in content area for now.

---

### 8.3 🟡 Project summary cards don't use the shared `KpiCard` or card tokens

**Status:** ✅ Partially fixed in Backlog Batch 2 (`9f23903`)

**Location:** `components/projects/project-overview-shell.tsx` — `ProjectSummaryCards`  
**Finding:** Added hover effects (`hover:shadow-md` + `-translate-y-0.5`) and transition classes to match dashboard card styling. Full KpiCard integration deferred to larger refactor.

---

### 8.4 🟡 Timeline page (`/projects/timeline`) has no visual preview in the navigation context

**Status:** 🔲 Backlog (requires new feature development)
...
**Status:** ✅ Fixed in Backlog Batch 1 (`a593a0f`)
...
**Location:** `app/(admin)/insights/reports/page.tsx`  
**Finding:** This sticky header used negative margin tricks to break out of the `mx-auto max-w-7xl` container. Extracted into reusable `<StickyPageHeader>` component with title, description, total, and actions slot.
...
**Finding:** Added `min-h-[400px]` to chart containers to prevent layout shift on tab switch.
...
- Mobile-responsive button list with aria-pressed

No action needed — the component was incorrectly identified as unaudited.

---

### 9.5 🟢 `ExportCsvButton` in reports has no visual feedback during export generation

**Status:** ✅ Already implemented (has `isPending` loading state)

**Finding:** The ExportCsvButton component already uses `useTransition` to show loading state during export.
...
**Status:** ✅ Already implemented (has Alert + disabled buttons)
...
No action needed — the page was incorrectly identified as a stub.

---

### 10.3 🟡 Settings pages have no `<h1>` — page identity relies entirely on the TopNav label

**Status:** ✅ Already implemented (all pages use `SettingsPageHeader`)

**Location:** Various `settings/*/page.tsx` files  
**Finding:** All settings sub-pages already use the `SettingsPageHeader` component with title and description.

---

### 10.4 🟡 User invite flow doesn't have a visual confirmation after invite is sent

**Status:** ✅ Already implemented (shows success Alert)

**Location:** `settings/users/invite/page.tsx`, `components/users/invite-form.tsx`  
**Finding:** The invite form already shows a success Alert after sending, with the invited email address and instructions.

---

### 10.5 🟢 Settings layout `220px` sidebar fixed width may be tight on smaller screens

**Status:** 🔲 Backlog (minor UX improvement)
...
**Status:** ✅ Already implemented (most buttons already use `disabled={isPending}`)

**Finding:** Most server actions already use `useTransition()` and disable buttons during transitions. Some buttons in dropdown menus don't have explicit loading states, but this is acceptable since the confirm dialog provides feedback.

---

### 11.5 🟢 `not-found.tsx` files are spread across sections — probably inconsistent with each other

**Status:** ✅ Already implemented (all use shared `NotFoundView`)

**Finding:** All `not-found.tsx` files already use the shared `NotFoundView` component from `components/ui/not-found-view.tsx`.
...
- `apps/admin/src/components/ui/pagination.tsx` — Reusable Pagination with page numbers
- `apps/admin/src/components/ui/sticky-page-header.tsx` — Reusable sticky page header
- `apps/admin/src/components/ui/route-error.tsx` — Shared error boundary component
...
- `apps/admin/src/app/(admin)/accounting/chart-of-accounts/loading.tsx` — Loading skeleton
- `apps/admin/src/app/(admin)/accounting/chart-of-accounts/error.tsx` — Error boundary
- `apps/admin/src/app/(admin)/accounting/general-ledger/loading.tsx` — Loading skeleton
- `apps/admin/src/app/(admin)/accounting/general-ledger/error.tsx` — Error boundary
- `apps/admin/src/app/(admin)/accounting/trial-balance/loading.tsx` — Loading skeleton
...
- `apps/admin/src/components/navigation/top-nav.tsx` — Two-level breadcrumb, theme toggle, StickyPageHeader
...
- `apps/admin/src/app/(admin)/billing/invoices/invoices-client.tsx` — Accessibility fix, shared status maps, Pagination
- `apps/admin/src/app/(admin)/billing/quotes/quotes-client.tsx` — Accessibility fix, status filter, Pagination
...
- `apps/admin/src/app/(admin)/insights/reports/page.tsx` — Dynamic PMG share rate, StickyPageHeader
- `apps/admin/src/components/reports/report-kpi-strip.tsx` — pmgShareRate prop
- `apps/admin/src/components/reports/reports-tabs.tsx` — pmgShareRate prop, min-h for charts
...
- `apps/admin/src/components/projects/project-overview-shell.tsx` — Renamed, hover effects, inline buttons
