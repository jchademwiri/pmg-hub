...
- **Waterfall** works for the _overall tender timeline_ because tenders have fixed, sequential phases: Requirements → Solution Design → Writing/Drafting → Review → Submission.
- **Kanban** (with WIP limits) works for _daily task execution_ because it prevents context-switching and keeps the solo worker focused.
