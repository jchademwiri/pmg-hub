# StudyEdge SA

### Brand Profile - Division 05 of Playhouse Media Group

---

## Who We Are

StudyEdge SA is the academic support division of Playhouse Media Group. We provide guided academic assistance, tutoring, research support, and study coaching to university and college students across South Africa.

StudyEdge SA is currently the smallest and newest division in the PMG group. In 2025, Jacob provided academic support to 1 student informally. The service is proven at a small scale - formalising it under a brand creates the structure to grow it intentionally alongside the other divisions.

---

## Quick Profile

| Field          | Detail                                               |
| -------------- | ---------------------------------------------------- |
| Brand Name     | StudyEdge SA                                         |
| Parent Company | Playhouse Media Group (PTY) Ltd                      |
| Division       | 05 - Academic Support & Tutoring                     |
| Status         | Active - 1 client in 2025, formalising for growth    |
| Domain         | studyedgesa.co.za _(to register)_                    |
| Email          | info@studyedgesa.co.za                               |
| Social Media   | @studyedgesa                                         |
| Revenue Model  | Per-session fees + per-assignment support + packages |
| 2025 Clients   | 1 student                                            |

---

## Tagline

> _"Your Academic Edge - Earned."_

---

## What We Do

StudyEdge SA helps South African students navigate the pressures of tertiary education through guided academic support, tutoring, research assistance, and structured study coaching. We work alongside students - helping them understand their material, structure their thinking, and produce work they are genuinely proud of.

---

## Important Positioning Note

StudyEdge SA is positioned as an **academic support and guided learning service** - not a ghostwriting or submission service. Services are framed around tutoring, guidance, editing, and support rather than producing work for direct student submission.

This positioning protects the business legally, is sustainable long-term, and opens doors to institutional partnerships and corporate learnerships that a ghostwriting-framed business cannot access.

---

## Services Offered

### Tutoring & Academic Coaching

| Service                               | Price             |
| ------------------------------------- | ----------------- |
| One-on-one tutoring session (1 hour)  | R250–R450/hr      |
| Subject-specific study plan (written) | R350              |
| Exam preparation coaching             | R300–R500/session |
| Monthly tutoring package (4 sessions) | R1,200–R1,600/mo  |

### Assignment & Research Support

| Service                                          | Price       |
| ------------------------------------------------ | ----------- |
| Assignment structure & guidance consultation     | R300–R500   |
| Research support - topic, sources, and outline   | R400–R700   |
| Draft review, feedback & editing                 | R350–R600   |
| Full research assistance (complex assignments)   | R800–R2,500 |
| Data analysis support (Excel, SPSS, basic stats) | R500–R1,500 |

### Academic Writing Support

| Service                                                 | Price       |
| ------------------------------------------------------- | ----------- |
| Essay structure & argument coaching                     | R350–R600   |
| Referencing & citation guidance (APA, Harvard, Chicago) | R250–R400   |
| Proofreading & language editing                         | R200–R450   |
| Presentation design & speaker notes                     | R500–R1,200 |

### Packages

| Package              | Included                                          | Price           |
| -------------------- | ------------------------------------------------- | --------------- |
| Assignment Starter   | Structure consult + research outline + 1 review   | R900            |
| Full Support Package | Research + draft feedback + editing + referencing | R2,000–R3,500   |
| Semester Support     | 8 tutoring sessions + unlimited email Q&A         | R3,500/semester |

---

## 2025 Reality & Growth Path

|                | 2025                     | 2026 Goal                                    |
| -------------- | ------------------------ | -------------------------------------------- |
| Clients        | 1 student (informal)     | 5–10 students                                |
| Revenue model  | Ad hoc, no fixed pricing | Structured packages + WhatsApp pipeline      |
| Marketing      | Word of mouth only       | WhatsApp, Instagram, student Facebook groups |
| Brand presence | None                     | studyedgesa.co.za + social media             |

> The biggest lever here is referrals. One satisfied student in a residence or study group typically knows 10 more. Formalising with a brand, a price list, and a WhatsApp catalogue is the low-cost path to growing this division.

---

## Target Market

- University and college students across South Africa
- Distance learning students (UNISA) studying without campus support
- Working adults studying part-time who need help managing deadlines
- Students in research-heavy disciplines - Business, Law, Education, Social Sciences
- Matric students preparing for university-level study

---

## Cross-Sell Opportunities

Students who graduate and start businesses feed directly back into the PMG ecosystem:

| If the client needs...                | Refer to...               |
| ------------------------------------- | ------------------------- |
| Company registration after graduating | LaunchPad SA              |
| A website for their new business      | Apex Web Solutions        |
| Logo or brand identity                | Playhouse Creative Studio |
| Tender compliance for their business  | Tender Edge Solutions     |

---

## Recommended Future Names

Current name **StudyEdge SA** is retained. Alternatives researched for future consideration:

| #   | Name              | Domain                   | Why Consider It                                                                     |
| --- | ----------------- | ------------------------ | ----------------------------------------------------------------------------------- |
| 1   | **PassMark Pro**  | passmarkpro.co.za        | "PassMark" is the universal student goal. Highly searchable and emotionally direct. |
| 2   | **ScholarHub SA** | scholarhubsa.co.za       | Positions the brand as a destination. Broad enough to cover all academic services.  |
| 3   | **GradeUp SA**    | gradeupsouthafrica.co.za | Outcome-focused and energetic. Strong for short-form social content.                |
| 4   | **AcadAssist SA** | acadassistsa.co.za       | Clean and professional. Better suited for LinkedIn and institutional channels.      |

> **Top search keywords:** assignment help South Africa · university tutoring Gauteng · UNISA assignment help · academic support SA · study help near me · tutoring services Pretoria · how to write a research proposal

---

## Brand Identity

| Element        | Detail                                            |
| -------------- | ------------------------------------------------- |
| Primary Colour | Deep Blue `#1E3A5F`                               |
| Accent Colour  | Sky Blue `#38BDF8`                                |
| Warm Accent    | Amber `#F59E0B`                                   |
| Display Font   | Syne Bold                                         |
| Body Font      | DM Sans                                           |
| Tone           | Supportive, intelligent, empowering, approachable |

---

## Positioning Statement

> StudyEdge SA gives South African students the academic support they need to understand their material, structure their thinking, and perform at their best - tutoring, research guidance, and academic coaching from people who genuinely want to see you succeed.

---

_Last updated: March 2026 | Version 2.0_
_StudyEdge SA - a Playhouse Media Group division_
_studyedgesa.co.za | info@studyedgesa.co.za_
