...
- **Validation Rules:** Assert that total Debits equal total Credits for each post. Ensure no journal entries are posted on invoice issue (operational-only AR). `Dr Bank / Cr Revenue` should be posted _only_ when payment is actually recorded.
