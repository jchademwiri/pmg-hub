...
export const chartOfAccounts = pgTable(
  ...
  (t) => [index('coa_code_idx').on(t.code), index('coa_type_idx').on(t.type)],
);
...
- **Spending Allocations:** If cash is actually spent _out_ of an allocation (e.g. paying professional fees or buying software), it should be posted as a standard expense (Dr Expense / Cr Bank) and categorized appropriately in the P&L.
