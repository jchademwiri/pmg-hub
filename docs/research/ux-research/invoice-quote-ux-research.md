...
- **Spanned Description Layout** — Instead of cramming the description into a single vertical column, the description field spans full-width _underneath_ the primary line item fields (Item, Qty, Rate, Total). This keeps the math columns aligned perfectly while giving massive space for detailed service descriptions.
