...
|---|--------|---------|--------|--------|
| S1 | **Add lock icon (🔒) to snapshot cards** | `snapshots-cockpit.tsx` | Low | ✅ Implemented |
| S2 | **Add delta indicators to single-period KPI cards** (reuse `SnapshotDeltaBadge`) | `snapshots-cockpit.tsx` | Low | ✅ Implemented |
| S3 | **Add ratio metrics** to detail panel: expense ratio, profit margin, PMG share ratio | `snapshots-cockpit.tsx` | Low | ✅ Implemented |
| S4 | **Add all-time metadata**: first period, last period, snapshot count, avg monthly revenue | `snapshots-cockpit.tsx` | Low | ⏳ Remaining |
| S5 | **Add mini sparklines** to sidebar snapshot cards (revenue trend) | `snapshots-cockpit.tsx` | Medium | ⏳ Remaining |

#### Phase 2: Enhanced Detail (1 week)

| # | Update | File(s) | Effort | Status |
|---|--------|---------|--------|--------|
| S6 | **Add Level 2 profit pool allocation bar** (Salary/Reinvest/Reserve/Flex) below the Level 1 gross split | `snapshots-cockpit.tsx` | Medium | ✅ Implemented |
| S7 | **Add drill-down side sheet** for snapshot metrics (click Revenue → income rows, click Expenses → expense rows) | New: `financial-drilldown-sheet.tsx` | Medium | ✅ Implemented |
| S8 | **Add "closed by" and "notes" fields** to snapshot metadata display | `snapshots-cockpit.tsx`, schema | Medium | ⏳ Remaining |
| S9 | **Add export/print** for individual snapshots (PDF or print-friendly view) | New: snapshot export | Medium | ⏳ Remaining |
| S10 | **Add hover tooltips** with additional context on all elements | `snapshots-cockpit.tsx` | Low | ✅ Implemented |

#### Phase 3: Close Month Wizard (1-2 weeks)

| # | Update | File(s) | Effort | Status |
|---|--------|---------|--------|--------|
| S11 | **Replace Close Month button with wizard dialog** (multi-step: summary → checks → confirm) | `close-month-button.tsx` → new wizard | High | ✅ Implemented |
| S12 | **Add pre-close integrity checks**: uncategorized expenses, draft invoices, reconciliation variance | New server queries + wizard | High | ✅ Implemented |
| S13 | **Add `isPeriodLocked` helper** to enforce lock-date guardrails in server actions | `packages/db/src/queries.ts`, actions | Medium | ⏳ Remaining |
| S14 | **Add `createdBy`, `status`, `notes` fields** to snapshots schema | `packages/db/src/schema/snapshots.ts` | Medium | ✅ Implemented |
| S15 | **Show status badges** on snapshot cards (locked, adjusted, needs review) | `snapshots-cockpit.tsx` | Low | ✅ Implemented |
...
| # | Update | File(s) | Effort | Status |
|---|--------|---------|--------|--------|
| R1 | **Add summary KPI strip** at top (Revenue, Expenses, PMG Share, Profit Pool with sparklines) | `reports/page.tsx` + new `ReportKpiStrip` | Low | ✅ Implemented |
| R2 | **Standardize chart colors** using HSL tokens (`--chart-1` through `--chart-5`) | `index.css` + chart components | Low | ✅ Implemented |
| R3 | **Add loading skeletons** for all chart cards | `reports/page.tsx` | Low | ⏳ Remaining |
| R4 | **Improve chart titles** to question-based format ("Which divisions drove revenue?") | Chart component headers | Low | ⏳ Remaining |

#### Phase 2: Enhanced Reports (1-2 weeks)

| # | Update | File(s) | Effort | Status |
|---|--------|---------|--------|--------|
| R5 | **Add report tabs** (Overview, Revenue, Expenses, Profit Pool) | `reports/page.tsx` + new tab components | Medium | ✅ Implemented |
| R6 | **Add period selector** (fiscal year + month range, not just year) | `year-filter.tsx` → `period-selector.tsx` | Medium | ⏳ Remaining |
| R7 | **Add MoM/YoY comparison toggles** | New: `comparison-toggle.tsx` | Medium | ⏳ Remaining |
| R8 | **Add drill-down side sheets** for chart data (click bar → see transactions) | New: `financial-drilldown-sheet.tsx` | Medium | ✅ Implemented |
| R9 | **Add PDF export** and print-friendly view | New: `export-pdf-button.tsx` | Medium | ⏳ Remaining |
| R10 | **Reorder charts**: Performance trend first, then allocations, then division/category breakdowns | `reports/page.tsx` | Low | ✅ Implemented |
...
| # | Update | Scope | Effort | Status |
|---|--------|-------|--------|--------|
| X1 | **Add sticky date range** in page header (global filter) | Both pages | Low | ⏳ Remaining |
| X2 | **Ensure all charts responsive** on mobile | All chart components | Medium | ✅ Implemented |
| X3 | **Add dark mode chart color tuning** (muted pastels) | `index.css` | Low | ✅ Implemented |
| X4 | **Add empty states with actionable CTAs** for all sections | Both pages | Low | ✅ Implemented |
| X5 | **Add keyboard shortcuts** (Cmd+K search, arrow navigation) | App-wide | Medium | ⏳ Remaining |
| X6 | **Add accessibility** (aria-labels, focus rings, alt text) | All components | Medium | ⏳ Remaining |
| X7 | **Fix PMG Share discrepancy** (align 20% docs with 25% code) | Docs + code | Low | ✅ Implemented |
...
1. **R1** — Add KPI strip to Reports page (✅ Implemented)
2. **S1-S3** — Lock icons, delta badges, ratio metrics on Snapshots (✅ Implemented)
3. **R4** — Question-based chart titles (⏳ Remaining)
4. **X7** — Fix PMG Share discrepancy (✅ Implemented)

### Sprint 2 (Next Week): Core Enhancements
5. **S6** — Level 2 allocation bar on Snapshots (✅ Implemented)
6. **R5-R7** — Report tabs (✅ Implemented), period selector & comparison toggles (⏳ Remaining)
7. **R10** — Reorder report charts (✅ Implemented)
8. **X1** — Sticky date range header (⏳ Remaining)

### Sprint 3 (Week 3): Drill-Down & Export
9. **S7** — Drill-down side sheet for Snapshots (✅ Implemented)
10. **R8** — Drill-down side sheets for Reports (✅ Implemented)
11. **R9** — PDF export (⏳ Remaining)
12. **S9** — Snapshot export/print (⏳ Remaining)

### Sprint 4 (Week 4): Close Workflow
13. **S11-S12** — Close Month wizard with integrity checks (✅ Implemented)
14. **S13-S14** — Database schema additions (✅ Implemented), guardrails (⏳ Remaining)
