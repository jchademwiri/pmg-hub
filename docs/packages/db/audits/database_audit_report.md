...
- **Current State**: PMG Hub has since migrated to the WebSocket-based serverless pool client (`drizzle-orm/neon-serverless`), which _does_ support fully interactive transactions via `db.transaction(async (tx) => { ... })`.
- **Audit Observation**:
  ...
  3. However, legacy comments still remain in files such as [posting.ts](file:///D:/websites/pmg-hub/apps/admin/src/lib/accounting/posting.ts#L98) stating: _"Uses db.batch() for atomicity..."_ when the actual implementation uses `db.transaction(...)`.
- **Strategic Stance**:
