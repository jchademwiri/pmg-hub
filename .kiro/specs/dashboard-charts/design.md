...
_For any_ row returned by `getMonthlyRevenueByDivision` or `getMonthlyFinancials`, the
...
_For any_ result array returned by `getMonthlyRevenueByDivision` or
...
_For any_ entry in the array returned by `getMonthlyFinancialsSeries`, both `revenue`
...
_For any_ raw DB result passed to the series-building transform in
...
_For any_ valid `getMoMSnapshot` result, `getMoMChartData` must return an array of
...
_For any_ array of N division names passed to `RevenueByDivisionChart`, the color
