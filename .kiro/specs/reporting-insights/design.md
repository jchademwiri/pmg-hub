...
export async function exportFinancialsCsv(year: number): Promise<string | { error: string }>;
```
...
_For any_ set of income and expense entries spanning multiple calendar years,
...
_For any_ calendar year that has at least one expense entry, `getExpensesByCategory(year)`
...
_For any_ valid four-digit year (1000–9999), `exportFinancialsCsv(year)` SHALL
...
_For any_ integer outside the range 1000–9999, `exportFinancialsCsv(year)` SHALL
return `{ error: 'Invalid year' }` and SHALL NOT throw. Additionally, _for any_
...
_For any_ string that is not a valid four-digit integer (i.e. does not match
...
fc.assert(
  fc.asyncProperty(fc.integer({ min: 1000, max: 9999 }), async (year) => {
    const result = await exportFinancialsCsv(year);
    if (typeof result !== 'string') return false;
    const lines = result.trim().split('\n');
    if (lines[0] !== 'Month,Revenue,Expenses,PMG Share,Profit Pool,Salary,Reinvest,Reserve,Flex')
      return false;
    if (lines.length !== 13) return false; // header + 12 months
    const eps = 0.01;
    for (const line of lines.slice(1)) {
      const [, rev, exp, pmg, profit, sal, reinv, res, flex] = line.split(',').map(Number);
      if (Math.abs(pmg - rev * 0.2) > eps) return false;
      if (Math.abs(profit - (rev - exp - pmg)) > eps) return false;
      if (Math.abs(sal - profit * 0.35) > eps) return false;
      if (Math.abs(reinv - profit * 0.3) > eps) return false;
      if (Math.abs(res - profit * 0.3) > eps) return false;
...
fc.assert(
  fc.asyncProperty(
    fc.oneof(fc.integer({ max: 999 }), fc.integer({ min: 10000 })),
    async (invalidYear) => {
