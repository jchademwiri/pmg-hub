...
export type MonthlyBudgetChartRow = {
  month: string;
  revenue: number;
  invoiced: number;
  expenses: number;
...
export async function getFinancialSummary(): Promise<FinancialSummary> {
  const [revenue, expenses, rates] = await Promise.all([
    getTotalRevenue(),
    getTotalExpenses(),
    getActiveRates(),
  ]);
...
export async function getLedgerBalances(): Promise<BucketBalances> {
  ...
  return {
    pmg_share: {
      expected: summary.pmgShare,
      spent: spentPmgShare,
      available: summary.pmgShare - spentPmgShare,
    },
  };
}
...
function buildDivisionSeries(rows: DivisionSeriesRow[]): DivisionSeriesChart {
...
export async function getBudgetChartSeriesForYear(year: number): Promise<MonthlyBudgetChartRow[]> {
