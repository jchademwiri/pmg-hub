...
import { AlertCircle, CheckCircle2, Mail, Receipt, Settings, Landmark } from 'lucide-react';
import { getAllDivisions, getAllDivisionBillingSettings } from '@pmg/db';
import { Card, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { SettingsPageHeader } from '@/components/settings/settings-page-header';
import { BillingSettingsClient, BankingOverviewMatrix } from './billing-settings-client';
...
export default async function BillingSettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="division-defaults" className="w-full">
        <TabsList className="w-full justify-start rounded-lg bg-muted/40 p-1">
          <TabsTrigger value="division-defaults" className="flex items-center gap-2">
            <Settings className="h-4 w-4" />
            Division Defaults
          </TabsTrigger>
          <TabsTrigger value="email-routing" className="flex items-center gap-2">
            <Mail className="h-4 w-4" />
            Email Routing & Delivery
          </TabsTrigger>
          <TabsTrigger value="banking-overview" className="flex items-center gap-2">
            <Landmark className="h-4 w-4" />
            Banking Overview
          </TabsTrigger>
        </TabsList>

        {/* Division Defaults Tab */}
        <TabsContent value="division-defaults" className="mt-4">
        ...
        {/* Email Routing & Delivery Tab */}
        <TabsContent value="email-routing" className="mt-4">
                      ...
                      <TableCell className="py-4">
                        {hasContactEmail ? (
                          ...
                          <span className="flex items-center gap-2 text-xs text-muted-foreground">
                            <AlertCircle className="shrink-0 text-amber-500" />
                            Not configured
                          </span>
                        )}
                      </TableCell>
        ...
        {/* Banking Overview Tab */}
        <TabsContent value="banking-overview" className="mt-4">
