...
export default async function FinanceIncomePage({ searchParams }: IncomePageProps) {
  ...
  const monthlySummaries = await getIncomeMonthlySummaries(
    fyStartYear,
    filters.divisionId,
    filters.clientId,
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <AlertCircle
              className={`size-4 ${globalUnallocated > 0 ? 'text-amber-500' : 'text-muted-foreground'}`}
            />
          </CardHeader>
          <CardContent>
            <div
              className={`text-2xl font-bold ${globalUnallocated > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}
            >
