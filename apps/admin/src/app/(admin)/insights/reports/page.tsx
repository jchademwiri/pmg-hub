...
export default async function ReportsPage({ searchParams }: ReportsPageProps) {
  ...
  const [
    ...
    getDistinctReportYears().catch((e) => {
      console.error('getDistinctReportYears failed:', e);
      return [] as number[];
    }),
    getMoMChartData().catch((e) => {
      console.error('getMoMChartData failed:', e);
      return [];
    }),
    getBudgetChartSeriesForYear(year).catch((e) => {
      console.error('getBudgetChartSeriesForYear failed:', e);
      return [];
    }),
    getExpensesByCategory(year).catch((e) => {
      console.error('getExpensesByCategory failed:', e);
      return [];
    }),
    getMonthlyFinancialsSeriesForYear(year).catch((e) => {
      console.error('getMonthlyFinancialsSeriesForYear failed:', e);
      return [];
    }),
    getLedgerBalances().catch((e) => {
      console.error('getLedgerBalances failed:', e);
      return undefined;
    }),
    getActiveRates().catch((e) => {
      console.error('getActiveRates failed:', e);
      return { pmg_share: 0.25 };
    }),
  ]);
