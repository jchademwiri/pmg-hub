...
import { AiGrowthCockpit } from '@/components/insights/ai-growth-cockpit';
...
export default async function AnalysisPage(props: { searchParams: Promise<{ year?: string }> }) {
  ...
  const selectedYear =
    Number.isFinite(parsedYear) && !Number.isNaN(parsedYear) ? parsedYear : defaultYear;
  ...
  if (divisionMetricsResult.status === 'rejected')
    console.error('Division metrics error:', divisionMetricsResult.reason);
  if (yoyComparisonResult.status === 'rejected')
    console.error('YoY comparison error:', yoyComparisonResult.reason);
  if (monthlyRevenueResult.status === 'rejected')
    console.error('Monthly revenue error:', monthlyRevenueResult.reason);
  if (clientConcentrationResult.status === 'rejected')
    console.error('Client concentration error:', clientConcentrationResult.reason);
  if (momBreakdownResult.status === 'rejected')
    console.error('MoM breakdown error:', momBreakdownResult.reason);
  ...
  const divisionMetrics =
    divisionMetricsResult.status === 'fulfilled' ? divisionMetricsResult.value : [];
  const yoyComparison = yoyComparisonResult.status === 'fulfilled' ? yoyComparisonResult.value : [];
  const monthlyRevenue =
    monthlyRevenueResult.status === 'fulfilled' ? monthlyRevenueResult.value : [];
  const clientConcentration =
    clientConcentrationResult.status === 'fulfilled' ? clientConcentrationResult.value : [];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <AnalysisTabs defaultTab="overview">
        <TabsList className="mb-4 flex w-full justify-start overflow-x-auto overflow-y-hidden lg:grid lg:w-[1080px] lg:grid-cols-7 h-auto hide-scrollbar border border-border/50">
          <TabsTrigger value="overview" className="flex-1 min-w-[110px]">
            Overview
          </TabsTrigger>
          <TabsTrigger
            value="ai-growth"
            className="flex-1 min-w-[130px] gap-1.5 text-blue-600 dark:text-blue-400 font-semibold"
          >
            <Sparkles className="h-3.5 w-3.5" /> AI Growth
          </TabsTrigger>
          <TabsTrigger value="details" className="flex-1 min-w-[110px]">
            Growth Trend
          </TabsTrigger>
          <TabsTrigger value="pipeline" className="flex-1 min-w-[140px]">
            Divisions & Pipeline
          </TabsTrigger>
          <TabsTrigger value="clients" className="flex-1 min-w-[100px]">
            Clients
          </TabsTrigger>
          <TabsTrigger value="mom" className="flex-1 min-w-[140px]">
            Month-over-Month
          </TabsTrigger>
          <TabsTrigger value="yoy" className="flex-1 min-w-[140px]">
            3-Year Comparison
          </TabsTrigger>
        </TabsList>

        <TabsContent value="ai-growth" className="space-y-6">
          <AiGrowthCockpit />
        </TabsContent>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Monthly cash income overlaid across previous financial years.
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="details" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                How invoiced revenue and payments received are trending, month to month, across FY{' '}
                {selectedYear}.
              </CardDescription>
            </CardHeader>
            <CardContent className="h-[400px]">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold">
                  {formatZAR(overview.pipeline.outstandingAR)}
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="clients" className="space-y-6">
          <Card className="border bg-card shadow-sm rounded-xl overflow-hidden">
            <CardHeader>
              ...
              <CardDescription>
                Income, invoiced amounts, and profit distribution across your client base for FY{' '}
                {selectedYear}.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <ClientConcentrationTable
        ...
        <TabsContent value="mom" className="space-y-6">
          <Card className="border bg-card shadow-sm rounded-xl overflow-hidden">
            <CardHeader>
              ...
              <CardDescription>
                Detailed monthly breakdown of invoiced revenue, cash income, expenses, and net
        ...
        <TabsContent value="yoy" className="space-y-6">
          <Card className="border bg-card shadow-sm rounded-xl overflow-hidden">
            <CardHeader>
              ...
              <CardDescription>
                Side-by-side metric comparison for the last three financial years.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <YoYComparisonTable
