...
export default async function AccountingOverviewPage({
  ...
  const [currentPeriod, periods, trialBalance, profitAndLoss, { data: recentEntries }] =
    await Promise.all([
      getCurrentOpenPeriod(),
      getAllAccountingPeriods(),
  ...
  const accountsReceivableBalance =
    trialBalance.find((r) => r.accountCode === '1100')?.balance ?? 0;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Accountant-grade double-entry bookkeeping system with journals, ledgers, and financial
          statements.
        </p>
      </div>
      ...
      <AccountingOverviewClient
        ...
        currentPeriod={
          currentPeriod ? { period: currentPeriod.period, status: currentPeriod.status } : null
        }
      />
    </div>
  );
