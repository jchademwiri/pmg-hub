  ...
  return (
    <div className="flex flex-col gap-6">
      <SetPageTotal
        value={
          data.netProfit >= 0
            ? `Net profit: R${data.netProfit.toFixed(2)}`
            : `Net loss: R${Math.abs(data.netProfit).toFixed(2)}`
        }
        variant={data.netProfit >= 0 ? 'green' : 'red'}
      />
      ...
      <ProfitAndLossClient data={data} periods={periods} selectedPeriod={params.period || ''} />
    </div>
  );
}
