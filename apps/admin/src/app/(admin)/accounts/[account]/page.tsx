...
import { getWithdrawalsByAccount, getAllIncome, getYTDSummary } from '@pmg/db'
...
export default async function AccountHistoryPage({ params }: AccountHistoryPageProps) {
  ...
  const [withdrawals, incomeEntries, ytd] = await Promise.all([
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        ...
        <span className={`ml-auto text-base font-bold tabular-nums ${currentBalance < 0 ? 'text-red-500' : ''}`}>
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        ...
        <div className="rounded-lg border px-4 py-2 flex flex-col gap-0.5">
          ...
          <span className="font-semibold tabular-nums text-amber-500">{formatZAR(totalWithdrawn)}</span>
        </div>
        <div className="rounded-lg border px-4 py-2 flex flex-col gap-0.5">
          <span className="text-xs text-muted-foreground">Balance</span>
          <span className={`font-semibold tabular-nums ${currentBalance < 0 ? 'text-red-500' : 'text-green-500'}`}>
            {formatZAR(currentBalance)}
          </span>
        </div>
      </div>
          ...
          <TableBody>
            {displayRows.map((row, i) => (
              <TableRow key={i}>
                ...
                <TableCell className={`text-right tabular-nums ${row.balance < 0 ? 'text-red-500' : ''}`}>
