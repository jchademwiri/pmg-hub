...
export function TimelineClient({ entries, clients, progressMap = {} }: TimelineClientProps) {
  ...
  const todayOffset = daysBetween(timelineStart.toISOString().split('T')[0], today);
        ...
        <div>
          <div className="w-full overflow-x-auto scrollbar-thin scrollbar-thumb-muted-foreground/20 relative group">
            <div
              ...
              style={
                {
                  '--desktop-min-width': `${Math.max(totalDays * 8, 800)}px`,
                ...
                {entries.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      {/* Mobile Timeline Dot */}
                      <div
                        className={`md:hidden absolute left-[7px] top-1/2 -translate-y-1/2 size-[10px] rounded-full border-2 border-background z-10 ${STATUS_DOT_COLORS[entry.status] || 'bg-muted-foreground'}`}
                      />
                      ...
                      <div className="flex-1 min-w-0 md:w-56 md:flex-none md:shrink-0 truncate flex flex-col justify-center">
                        <div className="flex items-center gap-2">
                          ...
                          {entry.priority === 'urgent' && (
                            <Badge
                              variant="destructive"
                              className="hidden md:inline-flex shrink-0 text-xs"
                            >
                              Urgent
                            </Badge>
                          )}
                        ...
                        <div className="flex items-start justify-between mt-0.5">
                          <div className="flex flex-col gap-1">
                            ...
                            <p className="md:hidden text-[10px] text-muted-foreground leading-none">
                              ...
                              {formatDate(entry.targetCompletionDate)}
                            </p>
                          </div>
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="text-[11px] text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
                                  ...
                                  <span>Buffer:</span>
                                  <span
                                    className={`font-medium text-right ${bufferDays < entry.bufferDays ? 'text-amber-500 font-semibold' : 'text-foreground'}`}
                                  >
