...
export default async function StatementDetailPage({ params, searchParams }: Props) {
  ...
  const currentBalance =
    transactions.length > 0 ? transactions[0]!.balance : adjustedOpeningBalance;
  ...
  const docPreviewProps = {
    ...
    org: buildOrgProps(
      ...
      divSettings,
      orgSettings,
      divSettings ? 'Playhouse Media Group' : null,
    ),
    client: {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 sm:gap-4">
        {[
          {
            label: 'Total Quoted',
            value: formatZAR(summary.totalQuoted),
            colorClass: 'text-muted-foreground',
          },
          {
            label: 'Total Invoiced',
            value: formatZAR(summary.totalInvoiced),
          ...
          },
          {
            ...
            colorClass:
              summary.totalPaid > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-foreground',
          },
          {
            label: 'Outstanding',
          ...
          },
          {
            ...
            colorClass:
              creditBalance > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-foreground',
          },
          {
            label: 'Conversion Rate',
            value: `${Math.round(summary.conversionRate * 100)}%`,
            colorClass:
              summary.conversionRate > 0 ? 'text-blue-500 dark:text-blue-400' : 'text-foreground',
          },
        ].map((s) => (
          <Card
            key={s.label}
            className="p-3 sm:p-4 flex flex-col justify-center gap-1 bg-muted/20 border border-border/50"
          >
            ...
            <CardContent>
              <div className="flex flex-col gap-2">
                {[
                  ...
                  {
                    label: '1–14 Days',
                    value: formatZAR(ageing.days1_14),
                    highlight: ageing.days1_14 > 0,
                  },
                  {
                    label: '15+ Days',
                    value: formatZAR(ageing.days15_30 + ageing.days31_60 + ageing.days61plus),
                    highlight: ageing.days15_30 + ageing.days31_60 + ageing.days61plus > 0,
                  },
                ].map((bucket) => (
                  <div
                    key={bucket.label}
                    className="flex justify-between items-center text-sm py-0.5 border-b border-border/40 last:border-b-0"
                  >
                    <span className="text-muted-foreground">{bucket.label}</span>
                    <span
                      className={`font-semibold tabular-nums ${bucket.highlight ? 'text-red-500 font-bold' : 'text-foreground'}`}
                    >
                      {bucket.value}
                    </span>
                  </div>
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="font-semibold text-sm truncate max-w-[65%]">
                    {inv.documentNumber}
                  </div>
                ...
                <div className="flex items-center justify-between text-[11px] sm:text-xs text-muted-foreground">
                  ...
                  <span className="tabular-nums">
                    Due: {fmtDate(inv.dueDate ?? inv.invoiceDate)}
                  </span>
                </div>
              </div>
            );
