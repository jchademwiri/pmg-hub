  ...
  return momData.map((item) => {
    ...
    const percentText =
      pct === null
        ? item.current === 0
          ? '0.0%'
          : 'not comparable from a zero base'
...
function buildFallbackCommentary(rows: VarianceRow[], context: CommentaryContext): string {
  ...
  const revenuePhrase =
    revenue.direction === 'flat'
      ...
      : revenue.direction === 'up'
        ...
        : 'decreased';
  const expensePhrase =
    expenses.direction === 'flat'
      ? 'were flat'
      : expenses.direction === 'up'
        ? 'increased'
        : 'decreased';
  const profitPhrase =
    profitPool.direction === 'flat'
      ? 'held steady'
      : profitPool.direction === 'up'
        ? 'improved'
        : 'weakened';
...
- Write one professional paragraph with no markdown bullets or greeting.`;
    ...
    const { text } = await generateText({
      ...
      system:
        'You are a careful financial controller. Ground every statement in the provided figures and avoid unsupported causal claims.',
      prompt,
      temperature: 0.2,
    });
    ...
    return {
      text: 'AI Gateway generation failed. Please check your AI_GATEWAY_API_KEY configuration.',
      isAi: false,
    };
  }
}
