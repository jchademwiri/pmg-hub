...
import { hasBillingLineItemItemIdColumn } from './line-item-compat';
...
/**
 * Core issue logic: issues a draft invoice and posts the journal entry atomically.
 * Includes auth check so it is safe as a public server action export.
 * Used by issueInvoice (UI action) and sendDocumentEmailAction.
 */
export async function issueInvoiceInternal(id: string): Promise<{ error?: string }> {
  await getSessionOrRedirect();
  const db = getDb();
...
async function voidInvoiceInternal(id: string): Promise<{ error?: string }> {
  ...
  const { reverseCreditApplication } = await import('@/app/actions/credit-management');
