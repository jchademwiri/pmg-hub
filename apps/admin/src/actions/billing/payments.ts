...
import { deleteIncome } from '@/actions/analytics/income';
import {
...
export async function recordClientPayment(
  ...
  try {
    ...
    const recordedIncomeId = await db.transaction(async (tx) => {
      ...
      if (excessAmount > 0) {
        const { createCreditNote } = await import('./credits');
    ...
    if (data.autoTransferPmgShare && data.amount > 0) {
      try {
        ...
        const { recordCashTransfer } = await import('@/actions/accounting/journals');
