...
export function MoMComparisonTable({ data, year, className }: MoMComparisonTableProps) {
  if (!data || data.length === 0) {
    return (
      <div
        className={cn(
          'flex h-[300px] items-center justify-center rounded-xl border bg-card text-muted-foreground shadow-sm',
          className,
        )}
      >
        ...
        <TableBody>
          {data.map((row) => {
            ...
            return (
              ...
              >
                <TableCell className="font-medium py-4">{row.monthLabel}</TableCell>
                ...
                <TableCell
                  className={`text-right font-semibold py-4 ${isPositiveProfit ? 'text-emerald-600 dark:text-emerald-500' : 'text-destructive'}`}
                >
                ...
                <TableCell className="py-4">
                  <div className="flex items-center justify-end gap-1.5 font-medium text-sm">
                    {row.momGrowth === null && <span className="text-muted-foreground">—</span>}
                    {isPositiveGrowth && (
                      <>
                        <TrendingUp className="h-4 w-4 text-emerald-500" />
                        <span className="text-emerald-600 dark:text-emerald-500">
                          +{row.momGrowth!.toFixed(1)}%
                        </span>
                      </>
                    )}
                    {isNegativeGrowth && (
      ...
      <div className="border-t bg-muted/30 p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-lg border bg-card p-3">
          ...
          <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">
            {formatZAR(data.reduce((s, r) => s + r.received, 0))}
          </p>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Avg{' '}
            {formatZAR(
              data.reduce((s, r) => s + r.received, 0) /
                Math.max(1, data.filter((r) => r.received > 0 || r.invoiced > 0).length),
            )}
            /mo
          </p>
        </div>
        ...
        <div className="rounded-lg border bg-card p-3">
          ...
          <p className="text-lg font-bold text-amber-600 dark:text-amber-400 tabular-nums">
            {formatZAR(data.reduce((s, r) => s + r.expenses, 0))}
          </p>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Avg{' '}
            {formatZAR(
              data.reduce((s, r) => s + r.expenses, 0) /
                Math.max(1, data.filter((r) => r.received > 0 || r.invoiced > 0).length),
            )}
            /mo
          </p>
        ...
        <div className="rounded-lg border bg-card p-3">
          ...
          <p className="text-lg font-bold text-foreground tabular-nums">
            {formatZAR(data.reduce((s, r) => s + r.netProfit, 0))}
          </p>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Avg{' '}
            {formatZAR(
              data.reduce((s, r) => s + r.netProfit, 0) /
                Math.max(1, data.filter((r) => r.received > 0 || r.invoiced > 0).length),
            )}
            /mo
          </p>
