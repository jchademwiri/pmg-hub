...
export function ClientConcentrationTable({ data, className }: ClientConcentrationTableProps) {
  if (!data || data.length === 0) {
    return (
      <div
        className={cn(
          'flex h-[400px] items-center justify-center rounded-xl border bg-card text-muted-foreground shadow-sm',
          className,
        )}
      >
              ...
              >
                <TableCell className="font-medium py-4">
                  <div className="flex items-center gap-2">
                    ...
                    {isHighRisk && (
                      <div
                        className="flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-600 dark:text-amber-500"
                ...
                <TableCell className="text-right py-4 font-medium text-blue-600 dark:text-blue-400">
                  {formatZAR(client.totalInvoiced ?? 0)}
                </TableCell>
                <TableCell className="text-right py-4 font-medium text-emerald-600 dark:text-emerald-500">
                  {formatZAR(client.totalIncome)}
                </TableCell>
                <TableCell className="text-right text-muted-foreground py-4">
                  {formatZAR(client.totalExpenses)}
                </TableCell>
                <TableCell
                  className={`text-right font-medium py-4 ${client.netProfit < 0 ? 'text-destructive' : 'text-emerald-600 dark:text-emerald-500'}`}
                >
