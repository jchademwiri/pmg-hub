...
function IncomeTableRow({
  entry, divisions, clients, deleteAction, updateAction,
}: {
  entry: IncomeRow
  divisions: { id: string; name: string }[]
  clients: { id: string; name: string; businessName: string | null }[]
  ...
  function startEdit() {
    ...
    setEditClientId(entry.clientId ?? ''); setEditDesc(entry.description ?? '')
  ...
  function handleSave() {
    ...
    startSaveTransition(async () => {
      ...
      fd.set('clientId', editClientId); fd.set('description', editDesc)
        ...
        <TableRow className="bg-muted/30">
          ...
          <TableCell>
            <Select value={editClientId} onValueChange={setEditClientId} disabled={isSaving}>
              <SelectTrigger className="w-40"><SelectValue /></SelectTrigger>
              <SelectContent>
                {clients.map((c) => <SelectItem key={c.id} value={c.id}>{c.businessName ?? c.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </TableCell>
          <TableCell>
...
export function IncomeTable({ entries, divisions, clients, deleteAction, updateAction }: IncomeTableProps) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {entries.map((entry) => (
          <IncomeTableRow
            ...
            divisions={divisions} clients={clients}
