...
export function ReportDocumentCanvas({
  ...
  return (
    <div
      id="printable-area"
      className="w-full bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 sm:p-10 shadow-xl transition-all"
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between pb-6 border-b border-zinc-200 dark:border-zinc-800 gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white uppercase">
            {companyName}
          </h1>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Division:{' '}
            <span className="font-semibold text-zinc-800 dark:text-zinc-200">{divisionName}</span>
        ...
        <div className="sm:text-right">
          <span
            ...
            className="inline-block px-3 py-1 text-xs font-semibold uppercase tracking-wider rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20"
          >
            {fullDocumentTitle}
          </span>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-2">
            Period:{' '}
            <span className="font-medium text-zinc-800 dark:text-zinc-200">{periodLabel}</span>
          </p>
          {dateRangeLabel && (
      ...
      <div className="mt-6">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              {data?.accounts?.map((acc: any) => (
                <tr key={acc.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                  ...
                  <td className="py-2.5 px-3 uppercase text-[11px] font-semibold text-zinc-500 dark:text-zinc-400">
                    {acc.type}
                  </td>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              {data?.entries?.map((je: any) => (
                <tr key={je.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                  ...
                  <td className="py-2.5 px-3 text-zinc-600 dark:text-zinc-400 tabular-nums">
                    {fmtDate(je.entryDate)}
                  </td>
                  <td className="py-2.5 px-3 font-medium text-zinc-800 dark:text-zinc-200">
                    {je.description || '—'}
                  </td>
                  <td className="py-2.5 px-3 text-zinc-500 dark:text-zinc-400 uppercase text-[10px]">
                    {je.sourceModule} / {je.sourceTable}
                  </td>
              ...
              {(!data?.entries || data.entries.length === 0) && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-zinc-400">
                    No journal entries found for selected filters.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              {data?.lines?.map((line: any) => (
                <tr key={line.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                  ...
                  <td className="py-2.5 px-3 font-medium text-zinc-800 dark:text-zinc-200">
                    <span className="font-mono text-xs mr-1.5 text-zinc-500">
                      {line.accountCode}
                    </span>
                    {line.accountName}
                  </td>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                {data?.rows
                  ?.filter(
                    (r: any) =>
                      Math.abs(r.debit ?? r.totalDebits ?? 0) >= 0.01 ||
                      Math.abs(r.credit ?? r.totalCredits ?? 0) >= 0.01,
                  )
                  .map((row: any) => (
                    <tr key={row.accountId} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                      ...
                      <td className="py-2.5 px-3 uppercase text-[10px] text-zinc-500">
                        {row.accountType}
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono tabular-nums font-semibold text-zinc-900 dark:text-zinc-100">
                        {(row.debit ?? row.totalDebits ?? 0) > 0
                          ? formatZAR(row.debit ?? row.totalDebits)
                          : '—'}
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono tabular-nums font-semibold text-zinc-900 dark:text-zinc-100">
                        {(row.credit ?? row.totalCredits ?? 0) > 0
                          ? formatZAR(row.credit ?? row.totalCredits)
                          : '—'}
                      </td>
                    </tr>
                  ))}
              ...
              {data && (
                <tfoot>
                  <tr className="border-t-2 border-zinc-900 dark:border-zinc-100 font-bold text-xs">
                    <td
                      colSpan={3}
                      className="py-3 px-3 uppercase tracking-wider text-zinc-900 dark:text-white"
                    >
                      Total Ledger Balance
                    </td>
                    <td className="py-3 px-3 text-right font-mono text-zinc-900 dark:text-white tabular-nums">
                      {formatZAR(data.totalDebits || 0)}
                    </td>
                    <td className="py-3 px-3 text-right font-mono text-zinc-900 dark:text-white tabular-nums">
                      {formatZAR(data.totalCredits || 0)}
                    </td>
                  </tr>
                </tfoot>
              )}
        ...
        {reportType === 'profit-and-loss' && (
          <div className="flex flex-col gap-4">
              ...
              <table className="w-full text-left text-xs border-collapse">
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  ...
                  {(!data?.statement?.revenue || data.statement.revenue.length === 0) && (
                    <tr>
                      <td colSpan={3} className="py-3 px-3 text-zinc-400 italic text-[11px]">
                ...
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    <td colSpan={2} className="py-2.5 px-3 text-zinc-900 dark:text-white uppercase">
                      Total Operating Revenue
                    </td>
                    <td className="py-2.5 px-3 text-right font-mono text-emerald-600 dark:text-emerald-400">
                      {formatZAR(data?.statement?.totalRevenue || 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  {data?.statement?.expenses?.map((row: any) => {
                    const isBadDebt =
                      row.accountCode === '5150' ||
                      row.accountName?.toLowerCase().includes('bad debt');
                    return (
                      <tr
                        key={row.accountId}
                        className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50"
                      >
                        <td className="py-2 px-3 font-mono text-zinc-500 w-20">
                          {row.accountCode}
                        </td>
                        <td className="py-2 px-3 font-medium text-zinc-800 dark:text-zinc-200">
                        ...
                        <td
                          className={`py-2 px-3 text-right font-mono ${isBadDebt ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-zinc-800 dark:text-zinc-200'}`}
                        >
                          {formatZAR(row.amount)}
                        </td>
                      </tr>
                ...
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    ...
                    <td className="py-2.5 px-3 text-right font-mono text-zinc-800 dark:text-zinc-200">
                      {formatZAR(data?.statement?.totalExpenses || 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div className="p-3.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 flex justify-between items-center border border-zinc-200 dark:border-zinc-800 mt-2">
              <span className="font-bold text-sm uppercase text-zinc-900 dark:text-white">
                Net Profit / (Loss)
              </span>
              <span
                className={`font-mono text-base font-bold ${
                  (data?.statement?.netProfit || 0) >= 0
        ...
        {reportType === 'division-performance' && (
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              {data?.divisions?.map((div: any) => (
                <tr key={div.divisionId} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                  ...
                  <td
                    className={`py-2.5 px-2 text-right font-mono tabular-nums font-bold ${
                      div.netProfit >= 0
                        ? 'text-emerald-600 dark:text-emerald-400'
                        : 'text-red-500 dark:text-red-400'
                    }`}
            ...
            {data?.divisions &&
              ...
              (() => {
                const totRev = data.divisions.reduce(
                  (s: number, d: any) => s + (d.totalRevenue || 0),
                  0,
                );
                const totInc = data.divisions.reduce(
                  (s: number, d: any) => s + (d.totalIncome || 0),
                  0,
                );
                const totAr = data.divisions.reduce(
                  (s: number, d: any) => s + (d.totalOutstandingAr || 0),
                  0,
                );
                const totExp = data.divisions.reduce(
                  (s: number, d: any) => s + (d.totalExpenses || 0),
                  0,
                );
                ...
                return (
                  <tfoot>
                    <tr className="border-t-2 border-zinc-900 dark:border-zinc-100 font-bold bg-zinc-50/50 dark:bg-zinc-900/40">
                      <td className="py-3 px-2 text-zinc-900 dark:text-white uppercase">
                        Total / Summary
                      </td>
                      <td className="py-3 px-2 text-right font-mono text-emerald-600 dark:text-emerald-400">
                        {formatZAR(totRev)}
                      </td>
                      <td className="py-3 px-2 text-right font-mono text-blue-600 dark:text-blue-400">
                        {formatZAR(totInc)}
                      </td>
                      <td className="py-3 px-2 text-right font-mono text-amber-600 dark:text-amber-400">
                        {totAr > 0 ? formatZAR(totAr) : '—'}
                      </td>
                      <td className="py-3 px-2 text-right font-mono text-zinc-800 dark:text-zinc-200">
                        {formatZAR(totExp)}
                      </td>
                      <td
                        className={`py-3 px-2 text-right font-mono ${totNet >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}
                      >
                        {formatZAR(totNet)}
                      </td>
                      <td className="py-3 px-2 text-right font-mono text-zinc-800 dark:text-zinc-200">
        ...
        {reportType === 'client-performance' && (
          <table className="w-full text-left text-xs border-collapse" suppressHydrationWarning>
            <thead>
              <tr
                className="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 uppercase tracking-wider text-[10px] font-bold"
                suppressHydrationWarning
              >
                ...
                <th className="py-2.5 px-2 text-right" suppressHydrationWarning>
                  Concentration %
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              ...
              {(!data?.clients || data.clients.length === 0) && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-zinc-400">
                    No client billing activity recorded.
                  </td>
                </tr>
              )}
            </tbody>
            {data?.clients &&
              ...
              (() => {
                const totRev = data.clients.reduce(
                  (s: number, c: any) => s + (c.totalRevenue || 0),
                  0,
                );
                const totCol = data.clients.reduce(
                  (s: number, c: any) => s + (c.totalCashCollected || 0),
                  0,
                );
                const totAr = data.clients.reduce(
                  (s: number, c: any) => s + (c.totalOutstandingAr || 0),
                  0,
                );
        ...
        {reportType === 'balance-sheet' && (
          <div className="flex flex-col gap-6">
            {/* ASSETS */}
            <div>
              <h4 className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2 border-b pb-1">
                Assets
              </h4>
              <table className="w-full text-left text-xs border-collapse">
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                ...
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    ...
                    <td className="py-2.5 px-3 text-right font-mono text-emerald-600 dark:text-emerald-400">
                      {formatZAR(data?.balanceSheet?.totalAssets || 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  {data?.balanceSheet?.liabilities?.map((row: any) => (
                    <tr key={row.accountId} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                      ...
                      <td className="py-2 px-3 text-right font-mono text-zinc-800 dark:text-zinc-200 font-semibold">
                        {formatZAR(row.amount)}
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    ...
                    <td className="py-2.5 px-3 text-right font-mono text-zinc-800 dark:text-zinc-200">
                      {formatZAR(data?.balanceSheet?.totalLiabilities || 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  ...
                  <tr className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 italic">
                    ...
                    <td className="py-2 px-3 text-right font-mono text-emerald-600 dark:text-emerald-400 font-semibold">
                      {formatZAR(data?.balanceSheet?.netIncome || 0)}
                    </td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    ...
                    <td className="py-2.5 px-3 text-right font-mono text-zinc-900 dark:text-white">
                      {formatZAR(data?.balanceSheet?.totalEquity || 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tfoot>
                  <tr className="border-t border-zinc-300 dark:border-zinc-700 font-bold">
                    <td className="py-2.5 px-3 text-zinc-900 dark:text-white uppercase">
                      Net Cash Flow from Operating Activities
                    </td>
                    <td
                      className={`py-2.5 px-3 text-right font-mono ${
                        (data?.cashFlow?.netOperatingCashFlow || 0) >= 0
                          ? 'text-emerald-600 dark:text-emerald-400'
                          : 'text-red-500 dark:text-red-400'
                      }`}
                    >
            ...
            <div className="p-3.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 flex justify-between items-center border border-zinc-200 dark:border-zinc-800">
              ...
              <span
                className={`font-mono text-base font-bold ${
                  (data?.cashFlow?.netCashIncrease || 0) >= 0
                    ? 'text-emerald-600 dark:text-emerald-400'
                    : 'text-red-500 dark:text-red-400'
                }`}
              >
            ...
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 bg-card flex flex-col gap-2">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="font-mono text-zinc-700 dark:text-zinc-300 font-semibold">
                  {formatZAR(data?.cashFlow?.startingCashBalance || 0)}
                </span>
              </div>
              <div className="flex justify-between items-center text-xs pt-2 border-t border-zinc-100 dark:border-zinc-900 font-bold">
                ...
                <span className="font-mono text-emerald-600 dark:text-emerald-400 text-sm">
                  {formatZAR(data?.cashFlow?.endingCashBalance || 0)}
                </span>
              </div>
            </div>
          </div>
        ...
        {reportType === 'changes-in-equity' && (
          <div className="flex flex-col gap-6">
            <div className="border-b-2 border-zinc-900 dark:border-zinc-100 pb-3 flex justify-between items-end">
              <div>
                <h2 className="text-base font-extrabold uppercase text-zinc-900 dark:text-white tracking-wide">
                  {companyName}
                </h2>
                <p className="text-xs text-zinc-500 font-medium mt-0.5">
                  Registration Number: 2023/683669/07
                </p>
              </div>
            ...
            <table className="w-full text-left text-xs border-collapse font-mono">
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                <tr>
                  <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                    Opening Balance (1 March)
                  </td>
                  <td className="py-2 px-2 text-right">100,00</td>
                  <td className="py-2 px-2 text-right">
                    {formatZAR(data?.afs?.statementOfChangesInEquity?.currentOpeningRetained || 0)}
                  </td>
                  <td className="py-2 px-2 text-right font-semibold">
                    {formatZAR(
                      100 + (data?.afs?.statementOfChangesInEquity?.currentOpeningRetained || 0),
                    )}
                  </td>
                </tr>
                <tr>
                  <td className="py-2 px-2 font-sans text-zinc-600 dark:text-zinc-400">
                    Net profit / (loss) for period
                  </td>
                  <td className="py-2 px-2 text-right">{formatZAR(0)}</td>
                  <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400 font-semibold">
                    {formatZAR(data?.afs?.statementOfChangesInEquity?.currentNetProfit || 0)}
                  </td>
                  <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400 font-semibold">
                    {formatZAR(data?.afs?.statementOfChangesInEquity?.currentNetProfit || 0)}
                  </td>
                ...
                <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-sm text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                  ...
                  <td className="py-3 px-2 text-right">
                    {formatZAR(data?.afs?.statementOfChangesInEquity?.currentClosingRetained || 0)}
                  </td>
                  <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                    {formatZAR(
                      100 + (data?.afs?.statementOfChangesInEquity?.currentClosingRetained || 0),
                    )}
                  </td>
                </tr>
              </tbody>
              ...
              <div className="flex flex-col gap-10">
                ...
                <div className="flex flex-col gap-6">
                  <div className="sticky top-13 sm:top-13 z-30 bg-white dark:bg-zinc-950 -mt-6 pt-6 pb-3 px-3 -mx-3 border-b-2 border-zinc-900 dark:border-zinc-100 flex justify-between items-end mb-4 shadow-sm text-zinc-900 dark:text-zinc-100">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 font-medium mt-0.5">
                        Registration Number: {info?.registrationNumber}
                      </p>
                    </div>
                    <div className="text-right">
                      <h3 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">
                      ...
                      <p className="text-xs text-zinc-500 font-medium">
                        for the year ended {info?.financialYearEnd}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-xs flex flex-col gap-4 text-zinc-800 dark:text-zinc-200">
                    <div>
                      <h4 className="font-bold uppercase text-zinc-500 text-[10px] tracking-wider mb-1">
                    ...
                    <div>
                      <h4 className="font-bold uppercase text-zinc-500 text-[10px] tracking-wider mb-2">
                        2. Business activities & Financial Results
                      </h4>
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900 font-mono">
                          <tr className="font-bold text-zinc-900 dark:text-white">
                            ...
                            <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400">
                              {formatZAR(dr?.businessActivities?.revenue?.current || 0)}
                            </td>
                            <td className="py-2 px-2 text-right text-zinc-400">
                              {formatZAR(dr?.businessActivities?.revenue?.prior || 0)}
                            </td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td className="py-2 px-2 text-right font-semibold text-zinc-900 dark:text-white">
                              {formatZAR(dr?.businessActivities?.operatingProfit?.current || 0)}
                            </td>
                            <td className="py-2 px-2 text-right text-zinc-400">
                              {formatZAR(dr?.businessActivities?.operatingProfit?.prior || 0)}
                            </td>
                          </tr>
                          <tr>
                            <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                            ...
                            <td className="py-2 px-2 text-right font-semibold text-emerald-600 dark:text-emerald-400">
                              {formatZAR(dr?.businessActivities?.netProfit?.current || 0)}
                            </td>
                            <td className="py-2 px-2 text-right text-zinc-400">
                              {formatZAR(dr?.businessActivities?.netProfit?.prior || 0)}
                            </td>
                          </tr>
                          <tr>
                            <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                            ...
                            <td className="py-2 px-2 text-right font-semibold text-zinc-900 dark:text-white">
                              {formatZAR(dr?.businessActivities?.totalAssets?.current || 0)}
                            </td>
                            <td className="py-2 px-2 text-right text-zinc-400">
                              {formatZAR(dr?.businessActivities?.totalAssets?.prior || 0)}
                            </td>
                          </tr>
                          <tr>
                            <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                            ...
                            <td className="py-2 px-2 text-right font-semibold text-amber-600 dark:text-amber-400">
                              {formatZAR(dr?.businessActivities?.totalLiabilities?.current || 0)}
                            </td>
                            <td className="py-2 px-2 text-right text-zinc-400">
                              {formatZAR(dr?.businessActivities?.totalLiabilities?.prior || 0)}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                    ...
                    <div className="pt-2 flex flex-col gap-2">
                      <h4 className="font-bold uppercase text-zinc-500 text-[10px] tracking-wider">
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900 font-mono">
                      <tr className="font-bold text-zinc-900 dark:text-white bg-zinc-50/50 dark:bg-zinc-900/30">
                        ...
                        <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(pnl?.revenue?.current || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.revenue?.prior || 0)}
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-700 dark:text-zinc-300">
                          Less: Depreciation expense
                        </td>
                        <td className="py-2 px-2 text-center font-sans text-zinc-400">Note 5</td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(pnl?.depreciation?.current || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.depreciation?.prior || 0)}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-700 dark:text-zinc-300">
                        ...
                        <td className="py-2 px-2 text-right">
                          {formatZAR(pnl?.employeeBenefits?.current || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.employeeBenefits?.prior || 0)}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-700 dark:text-zinc-300">
                        ...
                        <td className="py-2 px-2 text-right">
                          {formatZAR(pnl?.totalExpenses?.current || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.totalExpenses?.prior || 0)}
                        </td>
                      </tr>
                      <tr className="font-bold border-t border-b border-zinc-900 dark:border-zinc-100 text-zinc-900 dark:text-white">
                        <td className="py-2.5 px-2 font-sans uppercase">
                          Operating profit / (loss)
                        </td>
                        ...
                        <td className="py-2.5 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(pnl?.operatingProfit?.current || 0)}
                        </td>
                        <td className="py-2.5 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.operatingProfit?.prior || 0)}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-700 dark:text-zinc-300">
                      ...
                      <tr className="font-bold text-zinc-900 dark:text-white">
                        ...
                        <td className="py-2.5 px-2 text-center font-sans text-zinc-400"></td>
                        <td className="py-2.5 px-2 text-right">
                          {formatZAR(pnl?.profitBeforeTax?.current || 0)}
                        </td>
                        <td className="py-2.5 px-2 text-right text-zinc-400">
                          {formatZAR(pnl?.profitBeforeTax?.prior || 0)}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-700 dark:text-zinc-300">
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900 font-mono">
                      <tr className="font-bold text-zinc-900 dark:text-white uppercase text-[11px] bg-zinc-50 dark:bg-zinc-900/40">
                        <td colSpan={4} className="py-2 px-2 font-sans">
                          ASSETS
                        </td>
                      </tr>
                      <tr className="font-semibold text-zinc-800 dark:text-zinc-200 italic">
                        <td colSpan={4} className="py-1.5 px-3 font-sans">
                          Non-current assets
                        </td>
                      </tr>
                      {bs?.assets
                        ...
                        .map((a: any) => (
                          <tr key={a.accountId}>
                            <td className="py-1.5 px-5 font-sans text-zinc-700 dark:text-zinc-300">
                              {a.accountName}
                            </td>
                            <td className="py-1.5 px-2 text-center font-sans text-zinc-400">
                              Note 5
                            </td>
                      ...
                      <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                        ...
                        <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(bs?.totalAssets?.current || 0)}
                        </td>
                        <td className="py-3 px-2 text-right text-zinc-400">
                          {formatZAR(bs?.totalAssets?.prior || 0)}
                        </td>
                      </tr>

                      <tr className="font-bold text-zinc-900 dark:text-white uppercase text-[11px] bg-zinc-50 dark:bg-zinc-900/40">
                        <td colSpan={4} className="py-2 px-2 font-sans pt-4">
                      ...
                      <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-sm text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                        ...
                        <td className="py-3 px-2 text-center font-sans text-zinc-400"></td>
                        <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(bs?.totalLiabilitiesAndEquity?.current || 0)}
                        </td>
                        <td className="py-3 px-2 text-right text-zinc-400">
                          {formatZAR(bs?.totalLiabilitiesAndEquity?.prior || 0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="flex flex-col gap-6 pt-8 border-t-2 border-zinc-200 dark:border-zinc-800 print:break-before-page break-before-page">
                  ...
                  <table className="w-full text-left text-xs border-collapse font-mono">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                      <tr>
                        <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                          Balance at {eq?.priorYearStartLabel || '1 March 2025'}
                        </td>
                        <td className="py-2 px-2 text-right">100,00</td>
                      ...
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-600 dark:text-zinc-400">
                          Net profit / (loss) for FY{priYear}
                        </td>
                        <td className="py-2 px-2 text-right">{formatZAR(0)}</td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(eq?.priorNetProfit || 0)}
                        </td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(eq?.priorNetProfit || 0)}
                        </td>
                      ...
                      <tr className="font-bold border-t border-zinc-300 dark:border-zinc-700 bg-zinc-50/40 dark:bg-zinc-900/30">
                        <td className="py-2 px-2 font-sans">
                          Balance at {eq?.priorYearEndLabel || '28 February 2026'}
                        </td>
                        <td className="py-2 px-2 text-right">100,00</td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(eq?.priorClosingRetained || 0)}
                        </td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(100 + (eq?.priorClosingRetained || 0))}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans font-medium text-zinc-800 dark:text-zinc-200">
                          Balance at 1 March {priYear}
                        </td>
                        <td className="py-2 px-2 text-right">100,00</td>
                        <td className="py-2 px-2 text-right">
                          {formatZAR(eq?.currentOpeningRetained || 0)}
                        </td>
                        <td className="py-2 px-2 text-right font-semibold">
                          {formatZAR(100 + (eq?.currentOpeningRetained || 0))}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-2 font-sans text-zinc-600 dark:text-zinc-400">
                          Net profit / (loss) for FY{curYear} (to date)
                        </td>
                        <td className="py-2 px-2 text-right">{formatZAR(0)}</td>
                        <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400 font-semibold">
                          {formatZAR(eq?.currentNetProfit || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400 font-semibold">
                          {formatZAR(eq?.currentNetProfit || 0)}
                        </td>
                      ...
                      <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-sm text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                        <td className="py-3 px-2 font-sans uppercase">
                          Balance at {eq?.currentYearEndLabel || '28 February 2027'}
                        </td>
                        <td className="py-3 px-2 text-right">100,00</td>
                        <td className="py-3 px-2 text-right">
                          {formatZAR(eq?.currentClosingRetained || 0)}
                        </td>
                        <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(100 + (eq?.currentClosingRetained || 0))}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="flex flex-col gap-6 pt-8 border-t-2 border-zinc-200 dark:border-zinc-800 print:break-before-page break-before-page">
                  ...
                  <table className="w-full text-left text-xs border-collapse font-mono">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                      ...
                      <tr className="font-semibold text-zinc-800 dark:text-zinc-200 italic">
                        <td colSpan={4} className="py-1.5 px-5 font-sans">
                          Adjustments for:
                        </td>
                      </tr>
                      <tr>
                        <td className="py-1.5 px-8 font-sans text-zinc-600 dark:text-zinc-400">
                      ...
                      <tr className="font-bold border-t border-zinc-300 dark:border-zinc-700">
                        ...
                        <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(cf?.current?.netOperatingCashFlow || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(cf?.prior?.netOperatingCashFlow || 0)}
                        </td>
                      </tr>
                      ...
                      <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-sm text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                        ...
                        <td className="py-3 px-2 text-center font-sans text-zinc-400"></td>
                        <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(cf?.current?.endingCashBalance || 0)}
                        </td>
                        <td className="py-3 px-2 text-right text-zinc-400">
                          {formatZAR(cf?.prior?.endingCashBalance || 0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="flex flex-col gap-6 pt-8 border-t-2 border-zinc-200 dark:border-zinc-800 print:break-before-page break-before-page">
                  ...
                  <table className="w-full text-left text-xs border-collapse font-mono">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                      <tr className="font-bold text-zinc-900 dark:text-white uppercase text-[11px] bg-zinc-50 dark:bg-zinc-900/40">
                        <td colSpan={3} className="py-2 px-2 font-sans">
                          INCOME
                        </td>
                      </tr>
                      <tr className="font-bold text-zinc-900 dark:text-white">
                        ...
                        <td className="py-2 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(det?.revenue?.current || 0)}
                        </td>
                        <td className="py-2 px-2 text-right text-zinc-400">
                          {formatZAR(det?.revenue?.prior || 0)}
                        </td>
                      </tr>
                      ...
                      <tr className="font-bold text-zinc-900 dark:text-white uppercase text-[11px] bg-zinc-50 dark:bg-zinc-900/40">
                        <td colSpan={3} className="py-2 px-2 font-sans pt-4">
                          EXPENSES
                        </td>
                      </tr>
                      <tr>
                      ...
                      {det?.expenses?.map((exp: any) => {
                        const isBadDebt =
                          exp.accountCode === '5150' ||
                          exp.accountName?.toLowerCase().includes('bad debt');
                        return (
                          <tr key={exp.accountId}>
                            <td
                              className={`py-1.5 px-5 font-sans ${isBadDebt ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-zinc-700 dark:text-zinc-300'}`}
                            >
                              {exp.accountCode} - {exp.accountName}
                              {isBadDebt ? ' (write-off)' : ''}
                            </td>
                            <td
                              className={`py-1.5 px-2 text-right ${isBadDebt ? 'text-red-600 dark:text-red-400 font-semibold' : ''}`}
                            >
                      ...
                      <tr className="font-extrabold border-t-2 border-b-2 border-zinc-900 dark:border-zinc-100 text-sm text-zinc-900 dark:text-white bg-zinc-50 dark:bg-zinc-900/50">
                        ...
                        <td className="py-3 px-2 text-right text-emerald-600 dark:text-emerald-400">
                          {formatZAR(det?.netProfit?.current || 0)}
                        </td>
                        <td className="py-3 px-2 text-right text-zinc-400">
                          {formatZAR(det?.netProfit?.prior || 0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
