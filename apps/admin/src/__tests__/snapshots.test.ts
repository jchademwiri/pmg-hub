    ...
    fc.assert(
      fc.property(
        ...
        (revenue, expenses) => {
          ...
          const eps = 0.001;
          const actualProfit = revenue - expenses;
          const expectedPmgShare = Math.max(0, actualProfit) * 0.25;
          const expectedProfitPool = actualProfit;

          expect(Math.abs(s.pmgShare - expectedPmgShare)).toBeLessThan(eps);
          expect(Math.abs(s.profitPool - expectedProfitPool)).toBeLessThan(eps);
        },
      ),
      { numRuns: 100 },
    );
...
describe('Snapshots page unit tests', () => {
  ...
  it('renders empty-state message when snapshots = [] - Validates: Requirements 5.4, 5.10', () => {
    ...
    const emptyMessage =
      'Close a month from Dashboard to create your first locked monthly financial record.';
