...
// ─── Mocks ───────────────────────────────────────────────────────────────────
// Includes reorderTenderQueue so the same mock works for all three actions.
...
vi.mock('@pmg/db', () => ({
  getDb: () => mockGetDbBulk(),
  tenderScheduleEntries: { id: 'id', status: 'status' },
  eq: vi.fn(),
  and: vi.fn(),
  inArray: vi.fn(),
...
describe('bulkArchiveTenders', () => {
  ...
  it('returns count 0 for empty IDs', async () => {
    const { bulkArchiveTenders } = await import('@/app/actions/tender-schedule-bulk')
...
describe('bulkDeleteTenders', () => {
  ...
  it('returns count 0 for empty IDs', async () => {
    const { bulkDeleteTenders } = await import('@/app/actions/tender-schedule-bulk')
...
describe('reorderTenderQueue', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockGetSessionBulk.mockResolvedValue({ user: { id: 'user-1' } })
  })

  it('calls reorderTenderQueue with ordered IDs on success', async () => {
    const { reorderTenderQueue } = await import('@/app/actions/tender-schedule-reorder')
    mockReorderDb.mockResolvedValue(undefined)

    const result = await reorderTenderQueue(['id-1', 'id-2', 'id-3'])
  ...
  it('handles errors gracefully', async () => {
    ...
    const result = await reorderTenderQueue(['id-1'])
