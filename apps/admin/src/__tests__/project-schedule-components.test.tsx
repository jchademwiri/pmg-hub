...
vi.mock('@/app/actions/project-progress', () => ({
  ...
  renameProgressSectionAction: (sectionId: string, title: string) =>
    mockRenameSection(sectionId, title),
  ...
  toggleProgressItemAction: (itemId: string, isCompleted: boolean) =>
    mockToggleItem(itemId, isCompleted),
...
describe('ProjectFormDialog', () => {
  ...
  it('defaults division to __none__', async () => {
    await renderFormDialog();
    expect((document.getElementById('tender-division-hidden') as HTMLInputElement)?.value).toBe(
      '__none__',
    );
  });
