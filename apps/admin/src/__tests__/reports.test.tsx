...
vi.mock('@/components/reports/revenue-by-division-chart', () => ({
  RevenueByDivisionChart: () =>
    React.createElement('div', { 'data-testid': 'revenue-division-chart' }),
}));

vi.mock('@/components/reports/revenue-vs-expenses-chart', () => ({
  RevenueVsExpensesChart: () =>
    React.createElement('div', { 'data-testid': 'revenue-expenses-chart' }),
}));
...
describe('P3: CSV export correctness - structure and financial model', () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1000, max: 9999 }), async (year) => {
        ...
        for (const line of lines.slice(1)) {
          ...
          const [, rev, exp, pmg, profit] = parts.map((v, i) => (i === 0 ? v : Number(v))) as [
            string,
            number,
            number,
            number,
  ...
  it('P3: months with no DB data produce zero-value rows', async () => {
    ...
    for (const row of dataRows) {
      const [, rev, exp, pmg, profit] = row.split(',').map((v, i) => (i === 0 ? v : Number(v))) as [
        string,
        number,
        number,
        number,
