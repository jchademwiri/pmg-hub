...
describe('formatOrgAddress', () => {
  ...
  it('returns undefined when all address fields are null', () => {
    expect(
      formatOrgAddress({ addressStreet: null, addressCity: null, addressPostal: null }),
    ).toBeUndefined();
  });
  ...
  it('returns just the street when only that is present', () => {
    expect(
      formatOrgAddress({ addressStreet: '123 Main St', addressCity: null, addressPostal: null }),
    ).toBe('123 Main St');
  });

  it('handles empty strings', () => {
    expect(
      formatOrgAddress({ addressStreet: '', addressCity: 'Centurion', addressPostal: '' }),
    ).toBe('Centurion');
  });

  it('returns undefined when all fields are empty strings', () => {
    expect(
      formatOrgAddress({ addressStreet: '', addressCity: '', addressPostal: '' }),
    ).toBeUndefined();
  });
});
