...
vi.mock('@/app/actions/tender-schedule', () => ({
  createTenderScheduleEntry: (...args: any[]) => mockFormCreate(...args),
  updateTenderScheduleEntry: (...args: any[]) => mockEditUpdate(...args),
...
describe('TenderFormDialog', () => {
  ...
  async function renderFormDialog() {
    const { TenderFormDialog } = await import('@/components/scheduling/tender-form-dialog')
    return render(
      <TenderFormDialog clients={mockClients} divisions={mockDivisions} open={true} onOpenChange={mockOnOpenChange} />,
    )
  }
...
describe('TenderEditDialog', () => {
  ...
  async function renderEditDialog() {
    const { TenderEditDialog } = await import('@/components/scheduling/tender-edit-dialog')
    return render(
      <TenderEditDialog tender={mockTender} clients={mockClients} divisions={mockDivisions} onClose={mockOnClose} />,
    )
  }
