...
describe('leads test setup', () => {
  ...
  it('leadArb generates valid LeadRow shapes', () => {
    ...
    fc.assert(
      fc.property(leadArb, (lead) => {
        ...
        expect(lead.serviceInterest === null || typeof lead.serviceInterest === 'string').toBe(
          true,
        );
  ...
  it('P3: getAllLeads with divisionId filter returns only entries with that divisionId', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(leadArb), fc.uuid(), async (leads, divisionId) => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(leadArb), async (leads) => {
        ...
        const distinctSources = [
          ...new Set(leads.map((l) => l.source).filter((s): s is string => s !== null)),
  ...
  it('P9: updateLeadNotes round-trip - notes persisted, updatedAt updated', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(leadArb, fc.string({ maxLength: 1000 }), async (lead, notes) => {
  ...
  it('P11: LeadStatusSchema round-trip - all four valid status values parse correctly', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom('new', 'contacted', 'converted', 'lost'), (status) => {
