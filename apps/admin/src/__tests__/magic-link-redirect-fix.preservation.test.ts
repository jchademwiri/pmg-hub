...
describe('magic-link-redirect-fix - Property 2: Preservation - Baseline Behaviors', () => {
  ...
  describe('3.1 - Unauthenticated requests to protected paths redirect to /login', () => {
     ...
     */
    it('proxy redirects to /login when no session cookie is present on a protected path - Validates: Requirements 3.1', async () => {
    ...
    it('proxy redirects to /login for any protected path with no cookies - Validates: Requirements 3.1', async () => {
      const { proxy } = await import('@/proxy');
  ...
  describe('3.2 - Plain session cookie still grants access to protected paths', () => {
    ...
    it('proxy returns NextResponse.next() when plain better-auth.session_token cookie is present - Validates: Requirements 3.1', async () => {
      const { proxy } = await import('@/proxy');
  ...
  describe('3.3 - Allowlisted paths always pass through unconditionally', () => {
     ...
     */
    it('proxy returns NextResponse.next() for /login with no cookies - Validates: Requirements 3.2', async () => {
      const { proxy } = await import('@/proxy');
    ...
    it('proxy returns NextResponse.next() for /api/auth/signin with no cookies - Validates: Requirements 3.2', async () => {
      const { proxy } = await import('@/proxy');
    ...
    it('proxy returns NextResponse.next() for all /api/auth/* paths regardless of cookies - Validates: Requirements 3.2', async () => {
      const { proxy } = await import('@/proxy');
      const authPaths = [
        '/api/auth/signin',
        '/api/auth/signout',
  ...
  describe('3.4 - Rate limiting on /api/auth/ endpoints returns 429', () => {
     ...
     */
    it('proxy returns 429 after exceeding rate limit on /api/auth/ endpoint - Validates: Requirements 3.3', async () => {
      const { proxy } = await import('@/proxy');
