...
describe('Property 4: createLead contact requirement', () => {
  ...
  it('returns contact-required error and does not call db.insert when both email and phone are absent - Validates: Requirements 3.2', async () => {
    await fc.assert(
      fc.asyncProperty(
        // Non-empty name
  ...
  it('returns contact-required error and does not call db.insert when phone is empty string and email is absent - Validates: Requirements 3.2', async () => {
    await fc.assert(
      fc.asyncProperty(
        // Non-empty name
        fc.string({ minLength: 1, maxLength: 100 }).filter((s) => s.trim().length > 0),
