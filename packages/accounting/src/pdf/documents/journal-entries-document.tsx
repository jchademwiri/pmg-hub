      ...
      {isTruncated && (
        ...
        >
          <span style={{ fontSize: 7.5, color: '#b91c1c', fontWeight: 600 }}>
            Showing {entries.length} of {totalCount} journal entries. Narrow your filters to view
            more.
          </span>
        </div>
      )}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 8, color: '#27272a' }}>{entry.description}</span>
                  </div>
                  <Badge variant={statusVariant}>{entry.status.toUpperCase()}</Badge>
                </div>
                ...
                <Table>
                  <TableHeader style={{ backgroundColor: '#ffffff' }}>
                    <TableRow>
                      <TableHead
                        style={{ width: '20%', fontSize: 7, fontWeight: 700, color: '#64748b' }}
                      >
                        ACCOUNT CODE
                      </TableHead>
                      <TableHead
                        style={{ width: '50%', fontSize: 7, fontWeight: 700, color: '#64748b' }}
                      >
                  ...
                  <TableBody>
                    {lines.map((line, lIdx) => (
                      <TableRow key={`line-${line.id || lIdx}`}>
                        <TableCell
                          style={{ width: '20%', fontSize: 7.5, fontWeight: 600, color: '#0f172a' }}
                        >
                          {line.accountCode}
                        </TableCell>
                    ...
                    <TableRow
                      style={{ backgroundColor: '#fafafa', borderTop: '1px solid #e4e4e7' }}
                    >
                      <TableCell
                        style={{ width: '20%', fontWeight: 700, fontSize: 7.5, color: '#0f172a' }}
                      >
                        Entry Total
                      </TableCell>
