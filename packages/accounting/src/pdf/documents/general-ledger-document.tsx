    ...
    >
      {isTruncated && (
        ...
        >
          <span style={{ fontSize: 7.5, color: '#b91c1c', fontWeight: 600 }}>
            Showing {rows.length} of {totalCount} ledger entries. Narrow your filters to inspect
            remaining records.
          </span>
        </div>
      )}
      ...
      <Table>
        <TableHeader style={{ backgroundColor: '#f8fafc' }}>
          <TableRow>
            <TableHead style={{ width: '12%', fontSize: 7, fontWeight: 700, color: '#64748b' }}>
              DATE
            </TableHead>
            <TableHead style={{ width: '12%', fontSize: 7, fontWeight: 700, color: '#64748b' }}>
              ENTRY #
            </TableHead>
            <TableHead style={{ width: '14%', fontSize: 7, fontWeight: 700, color: '#64748b' }}>
              ACCOUNT
            </TableHead>
            <TableHead style={{ width: '34%', fontSize: 7, fontWeight: 700, color: '#64748b' }}>
              DESCRIPTION
            </TableHead>
            <TableHead
              style={{
          ...
          {rows.length === 0 ? (
            ...
            (rows ?? []).map((row, idx) => (
              <TableRow key={`gl-${idx}`}>
                ...
                <TableCell
                  style={{ width: '12%', fontSize: 7.5, fontWeight: 600, color: '#0f172a' }}
                >
                  {row.entryNumber}
                </TableCell>
