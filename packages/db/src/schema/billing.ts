...
export const billingDocumentTypeEnum = pgEnum('billing_document_type', ['quote', 'invoice']);

export const billingItemStatusEnum = pgEnum('billing_item_status', ['active', 'archived']);
...
export const documentSequences = pgTable(
  ...
  (t) => [
    unique('document_sequences_division_type_year_unique').on(t.divisionId, t.documentType, t.year),
  ],
);
...
export const invoices = pgTable(
  'invoices',
  {
    ...
    // recurring_invoice_id + billing_period: set when this invoice was generated by the
    // recurring billing run. billing_period is the "YYYY-MM" cycle the invoice covers.
    // Together with the partial unique index below, this prevents the same recurring
    // schedule from generating two invoices for the same month.
...
export const billingLineItems = pgTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`("discount_type" = 'percent' AND "discount_value" <= 100) OR "discount_type" IS DISTINCT FROM 'percent'`,
    ),
...
export const divisionBillingSettings = pgTable('division_billing_settings', {
  id: uuid('id').primaryKey().defaultRandom(),
  divisionId: uuid('division_id')
    .notNull()
    .unique()
